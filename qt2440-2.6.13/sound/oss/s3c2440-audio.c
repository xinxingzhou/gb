...
#define AUDIO_NAME		"QT2440_UDA1341"
#define AUDIO_NAME_VERBOSE	"QT2440 UDA1341 audio driver"
...
static void     start_bit2440_iis_bus_tx(void);
static void     start_bit2440_iis_bus_rx(void);
...
audio_clear_buf(audio_stream_t * s)
{
     ...
     */
    //bit2440_dma_flush_all(s->dma_ch);
...
static          ssize_t
bit2440_audio_write(struct file *file, const char *buffer, size_t count, loff_t * ppos)
{
...
static          ssize_t
bit2440_audio_read(struct file *file, char *buffer, size_t count, loff_t * ppos)
{
    const char     *buffer0 = buffer;
...
static unsigned int
bit2440_audio_poll(struct file *file, struct poll_table_struct *wait)
{
    unsigned int    mask = 0;
    int             i;
...
static          loff_t
bit2440_audio_llseek(struct file *file, loff_t offset, int origin)
{
    return -ESPIPE;
}

static int
bit2440_mixer_ioctl(struct inode *inode, struct file *file, uint cmd, ulong arg)
{
    /*
...
static int
bit2440_audio_ioctl(struct inode *inode, struct file *file, uint cmd, ulong arg)
{
    long            val;
...
//printk("a_ioctl = 0x%08x, SNDCTL_DSP_GETOSPACE=0x%08x\n", cmd, SNDCTL_DSP_GETOSPACE);
    ...
    case SNDCTL_DSP_SETDUPLEX:
        ...
        return -ENOSYS;
    default:
        return bit2440_mixer_ioctl(inode, file, cmd, arg);
    }
    return 0;
}

static int
bit2440_audio_open(struct inode *inode, struct file *file)
{
...
static int
bit2440_audio_release(struct inode *inode, struct file *file)
{
...
static struct file_operations bit2440_audio_fops = {
  llseek:  bit2440_audio_llseek,
  write:   bit2440_audio_write,
  read:    bit2440_audio_read,
  poll:    bit2440_audio_poll,
  ioctl:   bit2440_audio_ioctl,
  open:    bit2440_audio_open,
  release: bit2440_audio_release,
  owner:	THIS_MODULE
};

static struct file_operations bit2440_mixer_fops = {
  ioctl: bit2440_mixer_ioctl,
  owner: THIS_MODULE
};
...
#ifdef CONFIG_PM
static int
bit2440_audio_suspend(struct device *dev, u32 state, u32 level)
{
    switch (level)
    {
...
static int
bit2440_audio_resume(struct device *dev, u32 level)
{
    switch (level)
    {
    case RESUME_POWER_ON:
        printk("%s\n", __FUNCTION__);
        bit2440_uda1341_enable();
        break;
    }
...
#else
#define bit2440_audio_suspend	NULL
#define bit2440_audio_resume	NULL
#endif
...
static int bit2440_audio_probe(struct device *_dev)
{
    int             ret = 0;

    printk("QT2440 SOUND driver probe!\n");
    ...
    audio_dev_dsp = register_sound_dsp(&bit2440_audio_fops, -1);
    audio_dev_mixer = register_sound_mixer(&bit2440_mixer_fops, -1);
...
static int bit2440_audio_remove(struct device *_dev)
{
...
static struct device_driver bit2440_audio_driver = {
	.name		= "s3c2440-sound",
	.bus		= &platform_bus_type,
	.probe		= bit2440_audio_probe,
	.remove		= bit2440_audio_remove,
	.suspend	= bit2440_audio_suspend,
	.resume		= bit2440_audio_resume,
};

#ifdef USE_SYSFS
static int __init bit2440_uda1341_init(void)
{
	int ret = -ENODEV;
printk("QT2440 SOUND driver register\n");
	//if (!machine_is_h3600() || machine_is_h3100() || machine_is_h3800())
	ret = driver_register(&bit2440_audio_driver);
	if( ret )
	{
...
static void __exit bit2440_uda1341_exit(void)
{
	driver_unregister(&bit2440_audio_driver);
}
#else
int __init bit2440_uda1341_init(void)
{
    int             ret = 0;
...
void __exit bit2440_uda1341_exit(void)
{
    unregister_sound_dsp(audio_dev_dsp);
    unregister_sound_mixer(audio_dev_mixer);
    audio_clear_dma(&output_stream);
...
module_init(bit2440_uda1341_init);
module_exit(bit2440_uda1341_exit);
