...
#define NS_IN_KHZ 1000000		//fix by lili

static int s3c2410_nand_calc_rate(int wanted, unsigned long clk, int max)
{
	int result;

	result = (wanted * clk) / NS_IN_KHZ;		//fix by lili
	result++;
...
#define to_ns(ticks,clk) (((ticks) * NS_IN_KHZ) / (unsigned int)(clk))		//fix by lili
			       ...
			       struct device *dev)
{
	...
	int tacls, twrph0, twrph1;		//fix by lili
	unsigned long clkrate = clk_get_rate(info->clk);
	unsigned long cfg;

	/* calculate the timing information for the controller */
	clkrate /= 1000;		//fix by lili
