...
static int s3c2410sdi_probe(struct device *dev)
{
	...
	host->irq_cd		= IRQ_EINT16; // lili 2006-12-15, card detect irq
	...
	#else // bit board
	s3c2410_gpio_cfgpin(S3C2410_GPG8, S3C2410_GPG8_EINT16);		//add by lili 2006-12-15
	set_irq_type(host->irq_cd, IRQT_BOTHEDGE);
	#endif
	...
	mmc->max_seg_size = mmc->max_sectors << 12;		//change 9 to 12 by lili
